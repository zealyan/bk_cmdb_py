...
**⚠️ 不推荐修改UI源码，特别是接口和功能**

#### 2.1.1 核心原则
- **保持UI源码完整性**：尽可能不要修改 `/workspace/bk_cmdb_py/ui/` 目录下的代码
- **接口兼容性优先**：确保后端API与原有接口完全兼容，接口格式不能随意变更
- **向后兼容性**：任何后端变更都不能破坏现有前端功能
- **功能一致性**：前端功能应保持原样，仅通过后端适配实现需求

#### 2.1.2 可接受的UI修改场景
只有在以下**极端情况**下，才允许修改UI源码：
- **严重bug修复**：修复导致系统无法使用的严重错误
- **安全漏洞修复**：修复安全相关问题
- **极小的兼容性调整**：为了适配后端API的极小改动（如字段名微调）

#### 2.1.3 优先策略
当需要实现功能或修复问题时，按以下顺序选择解决方案：
1. **后端适配**：优先修改后端代码，使其兼容现有UI接口
2. **配置调整**：通过配置文件或环境变量调整，避免代码修改
3. **最小化UI修改**：确需修改UI时，只做必要的最小改动
4. **隔离测试**：任何UI修改都需要充分测试，确保不影响其他功能
