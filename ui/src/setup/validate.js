...
const buildInVaidationRules = {
  ...
  longchar: {
    validate: value => stringLength(value) <= 2000
  },
  businessTopoInstNames: {
    validate: (value) => {
      ...
      const values = value.split('\n')
      const list = values.map(text => text.trim()).filter(text => text)
      return list.every(text => pattern.test(text))
    }
  },
  reservedWord: {
...
const dictionary = {
  ...
  en: {
    messages: {
      ...
      longchar: () => 'Content length cannot exceed 2000 characters',
