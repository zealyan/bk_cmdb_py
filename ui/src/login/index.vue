...
<script>
  ...
  export default {
    ...
    mounted() {
      // 直接检查 window.User（index.html 中已预设）
      const isLoggedIn = !!(window.User && window.User.name)
      if (isLoggedIn) {
      ...
      async handleSubmit(event) {
        ...
        try {
          ...
          if (result && result.result) {
            ...
            const userInfo = {
              name: this.username,
              admin: this.username === 'admin' ? '1' : '0'
            }
            window.User = userInfo
            if (window.CMDB_APP && window.CMDB_APP.$store) {
              await window.CMDB_APP.$store.dispatch('login', userInfo)
              Vue.nextTick(() => {
                window.CMDB_APP.$router.push('/')
              })
            } else {
