...
export function getUniqueProperties(preset, dynamic) {
  // 过滤掉 undefined/null 元素
  const validPreset = preset.filter(Boolean)
  const validDynamic = dynamic.filter(Boolean)
  const unique = validDynamic.filter(property => !validPreset.includes(property))
  const full = [...validPreset, ...unique]
